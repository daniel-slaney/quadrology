...
function AABB:intersects( other )
	local xintersect = (other.xmin <= self.xmax) and (self.xmin <= other.xmax)
	local yintersect = (other.ymin <= self.ymax) and (self.ymin <= other.ymax)

	return xintersect and yintersect
end

-- Touching doesn't count as overlapping.
...
-- Sharing a corner does not count as abutting.
function AABB:abuts( other )
	local abutx = self.xmin == other.xmax or self.xmax == other.xmin
	local abuty = self.ymin == other.ymax or self.ymax == other.ymin
	local xoverlap = (other.xmin < self.xmax) and (self.xmin < other.xmax)
	local yoverlap = (other.ymin < self.ymax) and (self.ymin < other.ymax)

	return (abutx and yoverlap) or (abuty and xoverlap)
end
...
function AABB:scale( factor )
	...
	self.xmin = centre.x - (hw * factor)
	self.xmax = centre.x + (hw * factor)
	self.ymin = centre.y - (hh * factor)
	self.ymax = centre.y + (hh * factor)
end
